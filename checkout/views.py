...
from trailer.contexts import trailer_contents
...
def cache_checkout_data(request):
    try:
        ...
        stripe.PaymentIntent.modify(pid, metadata={
            'trailer': json.dumps(request.session.get('trailer', {})),
...
def checkout(request):
    ...
    if request.method == 'POST':
        ...
        if order_form.is_valid():
            ...
            order.trailer = json.dumps(trailer)
            order.save()
            for item_id, item_data in trailer.items():
                try:
                    ...
                    messages.error(request, (
                        "One of the cattle in your trailer wasn't found in our database. "
...
def checkout_success(request, order_number):
    ...
    if 'trailer' in request.session:
        del request.session['trailer']
